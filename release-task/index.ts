...
const { exec } = require("child_process");
 ...
 async function run() {
    ...
    await runCommandsOrThrow([
        ...
        `parapy app release . --url ${ parapyCloudAddress } -c ${ serviceAccountIdentifier } -s ${ serviceAccountSecret } -v ${ parapyAppVersion } --id ${ parapyAppIdentifier } --cloud-version default ${deploy ? " --deploy": "" }`
    ]);
 }
